...
import { VendorListPage as ShowCaseList } from 'pages/vendors';

function DashboardPage() {
  const { session } = useSession();

  const vendorStat = useSWR<PaginatedInterface<VendorInterface>>(
    () => `/vendors`,
    () => getVendors({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Vendors',
      result: vendorStat?.data?.totalCount,
      isLoading: vendorStat.isLoading,
    },
    {
      title: 'Products',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`vendors`} passHref legacyBehavior>
                  <Button
